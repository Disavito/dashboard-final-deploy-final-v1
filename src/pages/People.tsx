...
function People() {
  console.log("People component is rendering."); // Debugging log for preview error
  ...
  const fetchSocios = useCallback(async () => {
    if (!supabase) { // Defensive check for Supabase client
    ...
    // 2. Fetch all incomes to determine active status
    const { data: incomesData, error: incomesError } = await supabase
      .from('ingresos')
      .select('dni, amount, transaction_type'); // Select relevant fields
    ...
    const incomeMap = new Map<string, { hasPositive: boolean, hasNegative: boolean, totalCount: number }>();

    if (incomesData) {
      for (const income of incomesData) {
        if (income.dni) { // Ensure DNI exists for mapping
          const currentStatus = incomeMap.get(income.dni) || { hasPositive: false, hasNegative: false, totalCount: 0 };
    ...
    // 3. Enrich socios with isActive status
    const enrichedSocios: SocioTitular[] = (sociosData || []).map(socio => {
      const incomeStatus = socio.dni ? incomeMap.get(socio.dni) : undefined;
      let isActive = true; // Default to active if no income records or positive activity

      if (!incomeStatus || incomeStatus.totalCount === 0) {
        // No registered income: Active (as per requirement)
        isActive = true;
      } else if (incomeStatus.hasPositive) {
        // Has positive income (sale/income): Active
        isActive = true;
      } else if (incomeStatus.hasNegative && !incomeStatus.hasPositive) {
        // Only has negative income (returns/cancellations) and no positive income: Inactive
        isActive = false;
      }
      // If has both positive and negative, it's covered by hasPositive = true, so isActive remains true.

      return { ...socio, isActive };
    });
