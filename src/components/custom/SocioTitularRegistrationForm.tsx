                    ...
                    <FormItem className="flex flex-col sm:grid sm:grid-cols-4 sm:items-center gap-2 sm:gap-4">
                      ...
                      <Popover open={openLocalitiesPopover} onOpenChange={setOpenLocalitiesPopover}>
                        ...
                        <PopoverContent className="w-[--radix-popover-trigger-width] p-0 bg-card border-border rounded-xl shadow-lg">
                          <Command shouldFilter={false}> {/* Added shouldFilter={false} */}
                            <CommandInput
                            ...
                            />
                            <CommandList className="max-h-[200px] overflow-y-auto"> {/* Added max-h and overflow-y-auto */}
                              <CommandEmpty>No se encontró localidad.</CommandEmpty>
                              <CommandGroup>
                                {(() => {
                                  const searchTerm = watchedLocalidad.toLowerCase();
                                  const startsWithMatches = localitiesSuggestions.filter(loc => loc.toLowerCase().startsWith(searchTerm));
                                  const containsMatches = localitiesSuggestions.filter(loc =>
                                    loc.toLowerCase().includes(searchTerm) && !loc.toLowerCase().startsWith(searchTerm)
                                  );

                                  const combinedSuggestions = [...startsWithMatches, ...containsMatches];

                                  return combinedSuggestions.map((loc) => (
                                    <CommandItem
                                      value={loc}
                                      key={loc}
